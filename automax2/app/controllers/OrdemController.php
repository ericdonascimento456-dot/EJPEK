...
class OrdemController
{
    ...
    private function detalhe(int $idOs): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($acao === 'remover_item' && !in_array($os['status'], ['Finalizada', 'Cancelada'], true)) {
                if (!ehGerente()) {
                    flash('Apenas o Gerente pode remover peças de uma OS.', 'danger');
                } else {
                    $idItem = (int)($_POST['id_item'] ?? 0);
                    if ($idItem && $this->model->removerItem($idOs, $idItem)) {
                        flash('Peça removida e estoque devolvido.', 'success');
                    } else {
                        flash('Item não encontrado nesta OS.', 'danger');
                    }
                }
                header('Location: ' . BASE_URL . 'ordens.php?modo=detalhe&id=' . $idOs); exit;
            }
            ...
            if ($acao === 'finalizar' && !in_array($os['status'], ['Finalizada', 'Cancelada'], true)) {
                $this->model->finalizar($idOs, $totalItens);
                flash('Ordem de serviço #' . $idOs . ' finalizada com sucesso!', 'success');
                header('Location: ' . BASE_URL . 'ordens.php?modo=detalhe&id=' . $idOs); exit;
            }
        }
